...
export class SerorderService {
  ...
  url2 = this.urls+'category/'+ this.thefrominput + '?token='  +this.token;
  initialzeFormGroup(){
    this.form.setValue({
      category_id : null,
      category : '',
  ...
  deletedata(deleteid){
    ...
    if(confirm('Are you sure to delete this record ?')){
      // console.log(id);
      this.url2 = this.urls+'category/'+ this.thefrominput + '?token='  +this.token;
  ...
  updateUser(userdata) {
    ...
    this.thefrominput = userdata['category_id'];
    this.url2 = this.urls+'category/'+ this.thefrominput + '?token='  + this.token;
